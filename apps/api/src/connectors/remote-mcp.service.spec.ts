...
function harness(active: boolean, callTool = vi.fn().mockResolvedValue({
...
describe('RemoteMcpService', () => {
  ...
  it('maps notion_create_page and closes its client after success', async () => {
    ...
    expect(client.callTool).toHaveBeenCalledWith('notion-create-pages', {
  ...
  it('closes its client when the remote tool fails', async () => {
    const { client, service } = harness(
      true,
      vi.fn().mockRejectedValue(new Error('remote failed')),
    )

    await expect(service.withUserTools('user-1', tools => tools[0]!.execute(
