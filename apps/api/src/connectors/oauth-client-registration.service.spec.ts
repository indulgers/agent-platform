...
describe('OAuthClientRegistrationService', () => {
  it('stores the dynamic client identity and encrypted client secret', async () => {
    ...
    vi.mocked(prisma.oAuthClientRegistration.create).mockResolvedValue({
      ...
      clientSecretEncrypted: crypto.encrypt('registered-secret'),
      createdAt: new Date(),
      updatedAt: new Date(),
    })
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify({ client_id: 'registered-client', client_secret: 'registered-secret' }),
            { status: 201 },
          ),
        ),
    )

    await expect(service.getOrCreate('notion', callbackUrl, metadata)).resolves.toEqual({
      id: 'registration-1',
      clientId: 'registered-client',
  ...
  it('registers a public PKCE client when no durable registration exists', async () => {
    ...
    const fetch = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ client_id: 'registered-client' }), { status: 201 }),
      )
  ...
  it('rejects a failed registration response', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(new Response('registration unavailable', { status: 503 })),
    )

    await expect(service.getOrCreate('notion', callbackUrl, metadata)).rejects.toThrow(
      'OAuth client registration failed (503): registration unavailable',
    )
    expect(prisma.oAuthClientRegistration.create).not.toHaveBeenCalled()
  })

  it.each([{}, { client_id: '' }])(
    'rejects a registration response without a usable client ID: %j',
    async responseBody => {
      const { prisma, service } = createService()
      vi.mocked(prisma.oAuthClientRegistration.findUnique).mockResolvedValue(null)
      vi.stubGlobal(
  ...
  it('persists an empty dynamic client secret as null', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify({ client_id: 'registered-client', client_secret: '' }), {
            status: 201,
          }),
        ),
    )
    ...
    expect(
      vi.mocked(prisma.oAuthClientRegistration.create).mock.calls[0]![0].data.clientSecretEncrypted,
    ).toBeNull()
  })
  ...
  it('reuses a durable registration without calling the registration endpoint', async () => {
    ...
    await expect(service.getOrCreate('notion', callbackUrl, metadata)).resolves.toMatchObject({
      id: 'registration-1',
      clientId: 'registered-client',
    })
  ...
  it('uses the registration created by a concurrent request', async () => {
    ...
    vi.mocked(prisma.oAuthClientRegistration.create).mockRejectedValue(
      new Prisma.PrismaClientKnownRequestError('duplicate registration', {
        code: 'P2002',
        clientVersion: 'test',
      }),
    )
  ...
  it('loads a registration by its durable identity', async () => {
    ...
    await expect(service.byId('registration-1')).resolves.toEqual({
      id: 'registration-1',
      clientId: 'registered-client',
      clientSecret: undefined,
      callbackUrl,
    ...
    expect(prisma.oAuthClientRegistration.findUnique).toHaveBeenCalledWith({
      where: { id: 'registration-1' },
    })
  })

  it('loads the issuer without decrypting the client secret', async () => {
    const { prisma, crypto, service } = createService()
    vi.mocked(prisma.oAuthClientRegistration.findUnique).mockResolvedValue({
      id: 'registration-1',
      providerId: 'notion',
      ...
      issuer: metadata.issuer,
      clientId: 'registered-client',
      clientSecretEncrypted: 'malformed-legacy-ciphertext',
      createdAt: new Date(),
      updatedAt: new Date(),
    })
    const decrypt = vi.spyOn(crypto, 'decrypt')

    await expect(service.issuerById('registration-1')).resolves.toBe(metadata.issuer)
  ...
  it('creates a distinct registration when the authorization-server issuer changes', async () => {
    ...
    vi.mocked(prisma.oAuthClientRegistration.create)
      ...
      .mockResolvedValueOnce({
        ...
        providerId: 'notion',
        callbackUrl,
        issuer: newIssuerMetadata.issuer,
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ client_id: 'first-client' }), { status: 201 }),
        )
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ client_id: 'second-client' }), { status: 201 }),
        ),
    )
    ...
    expect(prisma.oAuthClientRegistration.findUnique).toHaveBeenNthCalledWith(1, {
      where: {
        providerId_callbackUrl_issuer: {
          providerId: 'notion',
          callbackUrl,
          issuer: metadata.issuer,
        },
    ...
    expect(prisma.oAuthClientRegistration.findUnique).toHaveBeenNthCalledWith(2, {
      where: {
        providerId_callbackUrl_issuer: {
          providerId: 'notion',
          callbackUrl,
